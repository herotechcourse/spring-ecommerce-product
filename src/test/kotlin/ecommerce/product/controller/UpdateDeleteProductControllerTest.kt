...
import ecommerce.product.helper.ProductTestFixture.ValidRequest.FLAT_WHITE
...
class UpdateDeleteProductControllerTest {
    ...
    fun `updateProduct() should be able to update product, and return 'ok 200' response`() {
        ...
        assertThat(product.name).isEqualTo(FLAT_WHITE.name)
        assertThat(product.price).isEqualTo(FLAT_WHITE.price.toPlainString())
        assertThat(product.imageUrl).isEqualTo(FLAT_WHITE.imageUrl)
    }
